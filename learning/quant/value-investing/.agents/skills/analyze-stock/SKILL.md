...
5. 进行估值。
   ...
   - FCFF DCF 强制关卡：在填写 DCF 目标价前，必须先建立逐年 Bear/Base/Bull 预测表，至少列示收入、收入增速、营业利润率、税率、折旧与摊销、资本开支、营运资本变动和 FCFF；每个 FCFF 数字都必须能由该表复算。
   - FCFF 增长率只能作为上述预测表的计算结果或明确的过渡假设，不得作为未解释的起始输入。若经营驱动数据不足，必须将 DCF 结果标为 `数据不足` 或 `未验证`，不得填写看似精确的目标价。
   - 在使用 `经营现金流－资本开支` 时，必须标注为简化 FCF 代理指标；不得称为 FCFF，除非已完成利息、税项及营运资本口径调整。
...
7. 验证并汇报。
   ...
   - 对 FCFF DCF，逐项核对预测表、企业价值到股权价值桥接、每股价值和 `stocks.md` 单元格；任一项缺失时不得发布 DCF 结果。
